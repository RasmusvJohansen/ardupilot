...
void SEKF::calculateKalmanGain()
{
    ...
    for(size_t row{0}; row < num_of_states; row++)
    {
        ...
        {
            numerator[row] += predict_error_cov[row][col] * output_lin_matrix[index][col];
            denominator[row] += output_lin_matrix[index][col] * predict_error_cov[row][col] * output_lin_matrix[index][row];
        }
        denominator[row] = denominator[row] + meas_noise_cov[index][index];
        kalman_gain[row][index] = numerator[row]/denominator[row];
    }
}
...
void SEKF::updateStateEstimate()
{
    for (size_t row{0}; row < num_of_states; row++)
    {
        state_estimate[row] = state_predict[row] + kalman_gain[row][index] * output_error[index];
    }
}

void SEKF::calculateOutputError()
{
    for (size_t row = 0; row < num_of_states; row++)
    {
        output_error[row] = output[row] - output_est[row];
    }
...
void SEKF::calculateStatePrediction()
{
    ...
    for (size_t row = 0; row < num_of_states; row++)
    {
        state_predict[row] = dynamics_vector[row] + observation_noise[row];
    }
}

void SEKF::updateOutputEstimate()
{
    for (size_t row = 0; row < num_of_states; row++)
    {
        output_est[row] = output_vector[row] + process_noise[row];
    }
}

void SEKF::dynamicsFunction()
{
...
void SEKF::calculateErrorCovariancePrediction()
{
    ...
    for (size_t row{0}; row < num_of_states; row++)
    {
        for (size_t col{0}; col < num_of_states; col++)
        {
            temp_error_cov[row][col] = (identity[row][col] - kalman_gain[row][index] * output_lin_matrix[row][col]);
            est_error_cov[row][col] = temp_error_cov[row][col] * predict_error_cov[row][col] * temp_error_cov[col][row] + kalman_gain[row][index]*meas_noise_cov[row][col]*kalman_gain[row][index];
        }
    }
}
...
void SEKF::calculateNextErrorCovariance()
{
    ...
    for (size_t row = 0; row < num_of_states; row++)
    {
        for (size_t col = 0; col < num_of_states; col++)
        {
            ...
            predict_error_cov[row][col] = predict_error_cov[row][col] + estimate_error_cov[row][col];
        }   
    }
}
